document.addEventListener("DOMContentLoaded", function() {
    ...
    document.querySelector("#image-caption").addEventListener("keyup", function(e) {
        if (e.keyCode === 13) generateMeme();
    });
    ...
    let downloadImage = ()=>{
        fetch(imageContainer.src).then((resp)=>resp.blob()).then((blobobject)=>{
    ...
    let generateMeme = ()=>{
        ...
        fetch(`https://api.imgflip.com/caption_image?template_id=393371323&username=N3-rd&password=1232112321&text1=${caption}`).then((res)=>res.json()).then((data)=>{
